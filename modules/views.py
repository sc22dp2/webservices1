...
def rate(request):
    ...
    professor = Professor.objects.get(prof_id = professor_id)
    instance = ModuleInstance.objects.get(professor = professor, module__code = module_code, year = year, semester = semester)
    already_exists = Rating.objects.get(module = instance, user=request.user)
    if already_exists:
        return HttpResponse("You have already rate this", status=401, content_type = "text/plain")
    rate = Rating.objects.create(module=instance, user=request.user, rating=rating)
    rate.save()
